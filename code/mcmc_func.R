    ...
    if(!injection){
        for(i1 in 1:Nw){
            ...
            for(j1 in nepoch){
                if(i1==1 | par.global!='trend'){
                    if(Npoly>0){
                        if(j1==1 | length(nepoch)==1 | any(grepl('^a',ep.par))){
                            nams <- c(nams,paste0(j1,'a',1:Npoly,'_',i1))
                        }
                    }
                    if(j1==1 | length(nepoch)==1 | any(grepl('^b',ep.par))){
                        nams <- c(nams,paste0(j1,paste0('b',1:Nb),'_',i1))
                    }
                }
                if(j1==1 | length(nepoch)==1 | any(grepl('^s',ep.par))){
                    nams <- c(nams,paste0(j1,paste0('s',1:Njitter),i1))
                }
                ...
                if((noise.model=='GP' | noise.model=='GPR') & !(gp.single & i1>1)){
                    nams <- c(nams,paste0(j1,c('sigma.red','l'),i1))
                    if(gp.type=='qp'){
                        nams <- c(nams,paste0(j1,c('Pgp','lp'),i1))
                    }
                }
                ...
                if(!all(Inds==0)){
                    if(j1==1| length(nepoch)==1){
                        nams <- c(nams,paste0(j1,paste0('c',1:length(Inds),'_'),i1))
                    }else if(any(grepl('^c',ep.par))){
                        int <- as.integer(gsub('c','',ep.par[grep('^c',ep.par)]))
                        nams <- c(nams,paste0(j1,paste0('c',int,'_'),i1))
                    }
                }
            }
        }
    ...
    for(i3 in 1:Nw){
        ...
        for(j1 in nepoch){
            if(i3==1 | par.global!='trend'){
                ...
                if(j1==1 | length(nepoch)==1 | any(grepl('^b',ep.par))){
                    nams <- c(nams,paste0(j1,'b',1:Nb,'_',i3,'[m/s]'))
                }
            }
	    if(j1==1 | length(nepoch)==1 | any(grepl('^s',ep.par))){
	        nams <- c(nams,paste0(j1,paste0('s',1:Njitter),i3,'[m/s]'))
	    }
...
###indices
            if(!all(Inds==0)){
                cname <- proxy.names
                if(j1==1| length(nepoch)==1){
                    nams <- c(nams,paste0(j1,paste0('c',cname[1:length(Inds)],'_'),i3,'[m/s]'))
                }else if(any(grepl('^c',ep.par))){
                    int <- as.integer(gsub('c','',ep.par[grep('^c',ep.par)]))
...
###
        ...
        if(Np.kep>0 & !noise.only){
            per.mul = as.numeric(pars.kep[grep('per([[:digit:]]{1})',names(pars.kep))])
            K.mul = as.numeric(pars.kep[grep('K([[:digit:]]{1})',names(pars.kep))])
            if(prior.kep!='e0'){
                e.mul = as.numeric(pars.kep[grep('e([[:digit:]]{1})',names(pars.kep))])
                Mo.mul = as.numeric(pars.kep[grep('Mo([[:digit:]]{1})',names(pars.kep))])
            }
            omega.mul = as.numeric(pars.kep[grep('omega([[:digit:]]{1})',names(pars.kep))])
            if(grepl('AK',kep.type)){
                logtau.mul = as.numeric(pars.kep[grep('logtau([[:digit:]]{1})',names(pars.kep))])
                ta.mul = as.numeric(pars.kep[grep('ta([[:digit:]]{1})',names(pars.kep))])
            }
            if(grepl('QP',kep.type)){
                logtq.mul = as.numeric(pars.kep[grep('logtq([[:digit:]]{1})',names(pars.kep))])
                aq.mul = as.numeric(pars.kep[grep('aq([[:digit:]]{1})',names(pars.kep))])
            }
...
###noise
        if(!kep.only){
            ...
            for(j1 in nepoch){
               ...
               if(Npoly>0){
                   ind <- grep(paste0(j1,'a\\d_',j2),par.can)
                   if(length(ind)>0){
                       at <- par.can[ind]
                   }else{
                       at <- par.can[paste0(nepoch[1],'a',1:Npoly,'_',j2)]
                   }
               }else{
                   at <- 0
               }
               ind <- grep(paste0(j1,'b\\d_',j2),names(par.can))
	       if(length(ind)>0){
	           bt <- par.can[ind]
	       }else{
	           bt <- par.can[paste0(nepoch[1],'b\\d_',j2)]
	       }
               if(par.global=='trend' & j2>1){
                   at <- pars.kep[grep(paste0(j1,'a\\d_1'),names(pars.kep))]
                   bt <- pars.kep[grep(paste0(j1,'b\\d_1'),names(pars.kep))]
               }
###include trend component the vector
               if(j2>1 & offset=='small'){
                  ind <- grep(paste0(j1,'b\\d_',1),names(pars.kep))
	          bt0 <- pars.kep[ind]
                  trend <- cal.trend(a=at,b=bt0+bt[1],t=tt[ind.ok]/time.unit)#tnorm[ind.ok,]
	       }else{
                  trend <- cal.trend(a=at,b=bt[1],t=tt[ind.ok]/time.unit)#tnorm[ind.ok,]
	       }
               if(Nb>1){
                   trend[ind.ins[[1]]] <- trend[ind.ins[[1]]]+bt[2]
               }
               dVr.kep[[ids[j2]]][ind.ok] <- dVr.kep[[ids[j2]]][ind.ok]+trend
...
####indices
               if(!all(Inds==0)){
                   ...
                   if(length(ind)>0){
                       cs <- par.can[ind]
                       if(length(cs)>1){
                           tmp <- rowSums(t(cs*t(proxies[ind.ok,])))
                       }else{
                           tmp <- cs*proxies[ind.ok]
                       }
...
arma <- function(pars,rv.kep,x=par.data[[1]]$RV,t=NA,j3=1,Np.kep=Np,ARMAtype=arma.type){
    ...
    for(j1 in nepoch){
        ...
        if(p.ar>0){
            ...
            for(j in 1:p.ar){
                if(ARMAtype=='abs'){
                    dVr.ar <- dVr.ar + c(rep(0,j),phi[j]*exp((tt[-(length(t)+1-(1:j))]-tt[-(1:j)])/exp(alpha))*y[-(length(t)+1-(1:j))])
                }
                if(ARMAtype=='squared'){
                    dVr.ar <- dVr.ar + c(rep(0,j),phi[j]*exp(-(tt[-(length(t)+1-(1:j))]-tt[-(1:j)])^2/(2*exp(alpha)^2))*y[-(length(t)+1-(1:j))])
                }
                if(ARMAtype=='index'){
                    phiT <- phiS[j]*(Sindex[-(1:j)]/max(Sindex))+phiC[j]
                    dVr.ar <- dVr.ar + c(rep(0,j),phiT*exp((tt[-(length(t)+1-(1:j))]-tt[-(1:j)])/exp(alpha))*y[-(length(t)+1-(1:j))])
                }
        ...
        if(q.ma>0){
            ...
            for(j in 1:q.ma){
                if(ARMAtype=='abs'){
                    dVr.ma <- dVr.ma + c(rep(0,j),w[j]*exp(-abs(tt[-(length(tt)+1-(1:j))]-tt[-(1:j)])/exp(beta))*res[-(length(tt)+1-(1:j))])
                }
                if(ARMAtype=='squared'){
                    dVr.ma <- dVr.ma + c(rep(0,j),w[j]*exp(-(tt[-(length(tt)+1-(1:j))]-tt[-(1:j)])^2/(2*exp(beta)^2))*res[-(length(tt)+1-(1:j))])
                }
                if(ARMAtype=='index'){
                    wT <- wS[j]*(Sindex[-(1:j)]/max(Sindex))+wC[j]
                    dVr.ma <- dVr.ma + c(rep(0,j),wT*exp((tt[-(length(tt)+1-(1:j))]-tt[-(1:j)])/exp(beta))*res[-(length(tt)+1-(1:j))])
                }
                if(ARMAtype=='index.exponent'){
...
#        for(j1 in 1:Nes[k3]){
        for(j1 in nepoch){
            if(any(grepl(paste0(j1,'s',k3),names(pars)))){
                s <- pars[grep(paste0(j1,'s\\d',k3),names(pars))]
            }else{
                s <- pars[grep(paste0(nepoch[1],'s\\d',k3),names(pars))]
            }
        if(noise.model=='GP'){
            ind.neg <- 0
            if(!gp.single){
                ...
                if(gp.type=='abs' | gp.type=='sq'){
                    ...
                    lp <- pars[paste0(j1,'lp',k3)]
                    ind.ok <- which(trv>epochs[j1] & trv<epochs[j1+1])
                    cov.red = rednoise.cov(sigma.red,l,tt=trv[ind.ok],tol=1e-10,Pgp=Pgp,lp=lp,type=gp.type)
                    diag(cov.red) <- diag(cov.red)+sigma^2+s^2
                }
                logdet.red <- unlist(determinant(cov.red,logarithm=TRUE))[1]
                logLike = try(logLike -1/2*logdet.red-1/2*(t(RV-RV.kep[[k3]])%*%chol2inv(chol(cov.red))%*%(RV-RV.kep[[k3]]))-length(trv)*log(sqrt(2*pi)),TRUE)
            }else{
                ts <- c(ts,trv)
                rvs <- c(rvs,RV)
                rvs.kep <- c(rvs.kep,RV.kep[[k3]])
                if(k3==Nw){
                    ind <- sort(ts,index.return=TRUE)$ix
                    ts <- ts[ind]
                    rvs <- rvs[ind]
                    rvs.kep <- rvs.kep[ind]
                    sigma.red = pars[paste0(j1,'sigma.red',1)]
                    l = pars[paste0(j1,'l',1)]
                    if(gp.type=='abs' | gp.type=='sq'){
                        cov.red = (cov.red.basic)^(1/l)*sigma.red^2
                        diag(cov.red) <- diag(cov.red)+eRV.all^2+s^2
                    }else if(gp.type=='qp'){
                        Pgp <- pars[paste0(j1,'Pgp',1)]
                        lp <- pars[paste0(j1,'lp',1)]
                        cov.red <- rednoise.cov(sigma.red,l,tt=trv.all,tol=1e-10,Pgp=Pgp,lp=lp,type=gp.type)
                        diag(cov.red) <- diag(cov.red)+eRV.all^2+s^2
                    }
                    logdet.red <- unlist(determinant(cov.red,logarithm=TRUE))[1]
                    logLike = try(logLike -1/2*logdet.red-1/2*(t(rvs-rvs.kep)%*%chol2inv(chol(cov.red))%*%(rvs-rvs.kep))-length(trv)*log(sqrt(2*pi)),TRUE)
                }
            }
            ...
            tmp <- 0
            for(i3 in 1:Njitter){
                tmp <- tmp+sum(dnorm(RV[ind.ins[[i3]]],mean=RV.kep[[k3]][ind.ins[[i3]]]+RV.arma[ind.ins[[i3]]],sd=sqrt(sigma[ind.ins[[i3]]]^2+s[i3]^2),log=T))
            }
            logLike = logLike +tmp
            ind.neg <- 0
            ...
            tmp <- 0
            for(i3 in 1:Njitter){
                tmp <- tmp+sum(dnorm(RV[ind.ins[[i3]]],mean=RV.kep[[k3]][ind.ins[[i3]]],sd=sqrt(sigma[ind.ins[[i3]]]^2+s[i3]^2),log=T))
            }
            logLike <- logLike+tmp
            ind.neg <- 0
...
#        Mo = pars[grep('Mo',names(pars))]
        if(prior.type!='e0'){
            eprior = 2*dnorm(e,mean=0,sd=Esd)#normalized semi-Gaussian distribution
        }else{
            eprior = 1
        }
        omegaprior = 1/(2*pi)
    ...
    for(i1 in 1:Nw){
        ...
        if(Npoly>0 & !is.null(amax)){
            logprior <- logprior+sum(log(1/(amax-amin)))
        }
        if(!is.null(bmax)){
            logprior <- logprior+sum(log(1/(bmax-bmin)))
        }
#        logprior <- logprior+ns*log(1/(smax-smin))
#        for(j1 in 1:Nes[i1]){
        for(j1 in nepoch){
	 ind <- grep(paste0(j1,'s\\d',i1),names(pars))
	 if(length(ind)>0){
	   s <- pars[ind]
	 }else{
           s <- pars[paste0(nepoch[1],'s\\d',j3)]
	 }
         if(prior.type=='jeffrey'){
         ...
         logprior = logprior+sum(log(sprior))
###Inds
         ind <- grep(paste0(j1,'c\\d{1,2}_',i1),names(pars))
         if(length(ind)>0){
             inds <- as.integer(gsub('.+c|_.+','',names(pars)[ind]))
             logprior <- logprior+length(inds)*log(1/(cmax-cmin))
         }
        if(noise.model=='TJ' | noise.model=='ARMATJ' | noise.model=='TJAR' | noise.model=='ARMATJAR'){
        ...
        if(noise.model=='TJAR' | noise.model=='ARMATJAR' | noise.model=='PSID' | noise.model=='ARMAPSID'){
            ...
            if(any(prior.names=='BIS') & any(prior.names=='FWHM') & TJAR.AB){
        ...
        if(noise.model=='GP' & !(gp.single & i1>1)){
            sigma.red <- pars[paste0(j1,'sigma.red',i1)]
            l <- pars[paste0(j1,'l',i1)]
            ...
            lprior = dnorm(l,mean=lmean,sd=lsd)
            logprior = logprior+log(redprior)+log(lprior)
            if(gp.type=='qp'){
                Pgp <- pars[grep('Pgp',names(pars))]
                lp <- pars[grep('lp',names(pars))]
#                lpprior <- 1/(lpmax-lpmin)
                lpprior <- dnorm(lp,mean=lp.mean,sd=lp.sd)
#                Pgpprior <- 1/(Pgpmax-Pgpmin)
                Pgpprior <- dnorm(Pgp,mean=Pgp.mean,sd=Pgp.sd)
                logprior = logprior+log(lpprior)+log(Pgpprior)
            }
        }
        if(noise.model=='ARMA' | noise.model=='ARMATJ' | noise.model=='ARMATJAR' | noise.model=='ARMAPSID'){
...
posterior <- function(param,noise.model){
    llike <- loglikelihood(param,noise.model)
    pr <- prior.func(param)
...
#    abline(v=c(x1per,x99per),col='blue')
    ...
    tmp <- c(x1per=x1per,x99per=x99per,x10per=x10per,x90per=x90per,xminus.1sig=xminus.1sig,xplus.1sig=xplus.1sig,mode=x1,mean=x2,sd=x3,skewness=x4,kurtosis=x5)
    return(tmp)
}
